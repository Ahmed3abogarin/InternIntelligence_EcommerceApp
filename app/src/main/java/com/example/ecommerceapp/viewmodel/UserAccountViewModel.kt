...
import com.example.ecommerceapp.EcommerceApp
...
class UserAccountViewModel @Inject constructor(
    ...
    private fun saveUserInformationWithNewImage(user: User, imageUri: Uri) {
        viewModelScope.launch {
            try { // to upload an image to firebaseStorage we gonna get byte array of this image first let get the bitmap image
                val imageBitMAp = MediaStore.Images.Media.getBitmap(getApplication<EcommerceApp>().contentResolver, imageUri)
                // next compress the image so we don't get a large image size
                val byteArrayOutputStream = ByteArrayOutputStream()
                imageBitMAp.compress(Bitmap.CompressFormat.JPEG,96,byteArrayOutputStream)
                val imageByteArray = byteArrayOutputStream.toByteArray()
                // finally save it
                val imageDirectory = storage.child("profileImages/${auth.uid}/${UUID.randomUUID()}")
                val result = imageDirectory.putBytes(imageByteArray).await() // await: gonna suspend this function
    ...
    private fun saveUserInformation(user: User, shouldRetriveOldImage: Boolean) {
        firestore.runTransaction{ transaction ->
            val documentRef = firestore.collection("Shop_users").document(auth.uid!!)
